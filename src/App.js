...
function App() {
  const {
    value: person,
    bind: bindPersonInput,
    reset: resetPersonInput
  } = useInput(undefined);
  const {
    value: days,
    bind: bindDaysInput,
    reset: resetDaysInput
  } = useInput(undefined);
  const { value: costInput, bind: bindCostInput } = useInput(undefined);
  ...
  const setPeopleArray = () => {
    if (person === undefined || person.length === 0) {
      console.log("Please insert friends name")
    } else if (days === undefined || days.length === 0) {
      console.log("No days have been inserted")
    } else {
  ...
  for (var i = 1; i < people.length; i++) {
    personPriceArray.push(
      people[i] +
        " owes " +
        (pricePerDayPerPerson * daysPerPerson[i]).toFixed(2)
    );
  }

  // pushing people and the amount of days they're seeing for user experience
  for (var p = 1; p < people.length; p++) {
    personAndDaysDisplayArray.push(
      people[p] + " is staying " + daysPerPerson[p] + " days, "
    );
  }
